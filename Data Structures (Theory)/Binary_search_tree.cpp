...
class bst{
    ...
    int sum_bst(Node *n,int sum){
        if(n==NULL){
            return 0;
        }
        else {
            int left_sum= sum_bst(n->left,sum);
            int r_sum=sum_bst(n->right,sum);
...
int main(){
    ...
    int sum = tree.sum_bst(tree.root,0);
    ...
    // cout<<"pre-order traversal after deletition : "<<endl;
    // tree.display_preorder(tree.root);
